...
Actions runner. Readers never need to care: `arrow::open_dataset()` walks
the whole tree and `query_canonical()` handles it for you.
...
Each snapshot carries *every* parameter ECCC serves for a station. Despite
the repo name, water temperature is a small slice of the archive — water
level and discharge dominate. This is the complete set (counts from the
2026-06-01 snapshot: 291 stations, ~19-month window):
