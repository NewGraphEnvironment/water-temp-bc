...
- Phase 4: three real-data OOM/schema failures fixed (window→arg_max→station-hash sharding; file-backed duckdb; typed fixtures) — decisive run at exact GHA profile 159s, golden PASS. Canonical live on S3 (30 objects, 0.37 GB — zstd 5× smaller than raw) + meta watermark. Lifecycle redirected to rtj (module owns it; CLI would be clobbered).
- Phase 5: query-helpers rewritten (canonical root, no read-time dedup, S3 timeouts, to_duckdb kept); query.R + README.Rmd updated; result.rds refreshed (3.9s); both outputs fully re-rendered + content-verified; live E2E: latest-per-station 1.3s (was minutes). Finding: bucket policy lacks s3:ListBucket → anonymous open_dataset never worked; joins lifecycle in the rtj s3-module issue.
- Next: /code-check, /planning-archive, /gh-pr-push; post-merge compact_only dispatch; rtj issue (lifecycle + ListBucket), pending user go
