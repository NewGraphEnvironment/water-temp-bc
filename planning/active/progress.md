...
- **Phase 4 done (pending merge-to-main verification):** rtj #147 merged + applied; role ARN `arn:aws:iam::414155577829:role/role_gha_water_temp_bc` posted on issue #17. Wrote `.github/workflows/snapshot.yml` (monthly cron + workflow_dispatch, OIDC auth, setup-r-dependencies → snapshot.R → `aws s3 cp`). Pinned `ngr` ref in DESCRIPTION (`@1e5758fee8992b1e7421db01166ba7f4e1d7f5d2`).
- **Next:** Phase 5 — open PR; after merge, trigger `workflow_dispatch` once from main to verify end-to-end; close #14, #13; archive PWF.
